...
def main():
    ...
    p_pipeline.add_argument(
        "--json_logs", action="store_true", help="Output logs in JSON format."
    )
    ...
    p_train_eval.add_argument("--algorithm", choices=["rf", "logreg"], default="rf")
    p_train_eval.add_argument("--test_size", type=float, default=0.25)
    p_train_eval.set_defaults(
        func=train_eval_main, verbose=False, json_logs=False, dry_run=False
    )
    ...
    p_train_filter.set_defaults(
        func=train_filter_main, verbose=False, json_logs=False, dry_run=False
    )
    ...
    p_analyze_ev.set_defaults(
        func=analyze_ev_main, verbose=False, json_logs=False, dry_run=False
    )
    ...
    p_summarize_matches.add_argument(
        "--output_csv", required=True, help="Path to save the match-level summary."
    )
    p_summarize_matches.add_argument("--top_n", type=int, default=10)
    p_summarize_matches.set_defaults(
        func=summarize_matches_main, verbose=False, json_logs=False, dry_run=False
    )
    ...
    p_summarize_tourneys.set_defaults(
        func=summarize_tournaments_main, verbose=False, json_logs=False, dry_run=False
    )
    ...
    p_plot.set_defaults(
        func=plot_leaderboard_main, verbose=False, json_logs=False, dry_run=False
    )

    args = parser.parse_args()

    # Call the appropriate function with the parsed args
    args.func(args) if args.command != "pipeline" else args.func(**vars(args))
