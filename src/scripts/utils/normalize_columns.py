...
def normalize_columns(df: pd.DataFrame) -> pd.DataFrame:
    ...
    **Canonical columns:**
    ...
    """
    df = df.rename(columns={k: v for k, v in CANONICAL_RENAME_MAP.items() if k in df.columns})
    return df
...
def assert_required_columns(df: pd.DataFrame, required=CANONICAL_REQUIRED_COLUMNS, context="") -> None:
